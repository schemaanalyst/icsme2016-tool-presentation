...
\begin{tikzpicture}[thick,scale=1, every node/.style={scale=1}]
  ...
  \path[use as bounding box] (-3.25,2.5) rectangle (10,-2);

    \path[->]<1-> node[]
	    (sa) at (1.5,.75) {\includegraphics[width=3cm]{content/sa_logo.png}};

    \path[->]<2-> node[io, left of=sa, yshift=0.70in,
    xshift=-1.25in,text width=8.5ex]
    (cc) {\centering Coverage Criterion} (cc) edge node {} (sa);

    \path[->]<3-> node[io, left of=sa, yshift=0.0in,
    xshift=-1.25in,text width=9.0ex]
    (dg) {\centering Data Generator} (dg) edge node {} (sa);

    \path[->]<4-> node[io, left of=sa, yshift=-0.70in,
    xshift=-1.25in,text width=8.5ex]
    (ds) {\centering Database Schema} (ds) edge node {} (sa);

    \path[->]<5-> node[io, right of=sa, yshift=0.0in,
    xshift=1.25in,text width=8.5ex]
    (ts) {\centering JUnit Test~Suite} (sa) edge node {} (ts);

    \path[->]<6-> node[special,below of=sa, yshift=-1.0in,text width=24em] (takeaway) {Extensible tool for test data generation};

    % \path[->]<6-> node[io, left of=sa, yshift=-1.40in,
    % xshift=-1.25in,text width=8.5ex]
    % (aq) {\centering Annotated \\ Queries} (aq) edge node {} (sa);
