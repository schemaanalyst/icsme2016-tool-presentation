...
\begin{tikzpicture}[thick,scale=1, every node/.style={scale=1}]
  ...
  \path[use as bounding box] (-3.0,2.5) rectangle (10,-2);
    ...
    \path[->]<1-> node[io, left of=sa, yshift=0.70in,
    xshift=-1.25in,text width=8.5ex]
    (cc) {\centering Coverage \\ Criterion} (cc) edge node {} (sa);

    \path[->]<1-> node[io, left of=sa, yshift=0.0in,
    xshift=-1.25in,text width=8.5ex]
    ...
    \path[->]<1-> node[io, right of=sa, yshift=0.0in,
    xshift=1.25in,text width=8.5ex]
    (ts) {\centering Test \\ Suite \\} (sa) edge node {} (ts);

    \path[->]<2-> node[io, left of=sa, yshift=-1.40in,
    xshift=-1.25in,text width=8.5ex]
