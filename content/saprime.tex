...
\begin{tikzpicture}[thick,scale=0.85, every node/.style={scale=0.85}]
    \node (wa) [wa]  {Query-aware \textit{SchemaAnalyst}};
    \path (wa.west)+(-\blockdist,-.625) node (asr1) [sensor] {Database Schema};
    \path (wa.west)+(-\blockdist,-1.875) node (queries) [sensor] {Annotated
    Queries};

    \path (wa.west)+(-\blockdist,1.875) node (asr2)[sensor] {Coverage Criterion};
    \path (wa.west)+(-\blockdist,0.625) node (dots)[sensor] {Data Generator};
    ...
    \path [draw, ->] (asr1.east) -- node [above] {}
        (wa);
    \path [draw, ->] (queries.east) -- node [above] {}
        (wa);
    ...
    %
\end{tikzpicture}
\caption{\label{fig:schemaanalystP}The inputs and outputs of the proposed 
Query-aware \textit{SchemaAnalyst}.}
\end{figure}
